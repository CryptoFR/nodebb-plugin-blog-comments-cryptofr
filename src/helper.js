...
const getTopicPids = (tid) => new Promise((resolve, reject) => {
    // We want all the posts here
    posts.getPidsFromSet(`tid:${tid}:posts`, 0, -1, false, function data (err, res) {
        if (err) {
            reject(err);as
        } else {
            resolve(res);
...
const getPostsData = (pids) => new Promise((resolve, reject) => {
...
const isInCache = (tid) => new Promise((resolve, reject) => {
...
const addPostData = (posts, uid) => new Promise((resolve, reject) => {
...
const getPostsFromCache = (tid) => new Promise((resolve, reject) => {
    db.getSortedSetRange(getCacheKey(tid), 0, -1, function sortedSetRange(err, data) {
        if (err) {
            reject(err);
        } else {
...
const getNestedPosts = async (tid, uid) => {
    ...
    for (let i = 0; i < posts.length; i++) {
        ...
        p.isReply = p.hasOwnProperty('toPid') && parseInt(p.toPid) !== parseInt(tid) - 1;
