...
import { bindEvents, prepareSignout, loadMoreEvent, newerCommentsDisplayEvent, markdownSpecialActions, moveCaptchaBox } from './events.js';
...
export function drawComments(res = null, i = 0) {
    ...
    if (XHR.status >= 200 && XHR.status < 400) {
        ...
        setTimeout(function() {
            ...
            var loginModal = prepareModal(data.loginModalTemplate, data.token, onSubmitLogin);
            addSocialAuthListeners(loginModal);
            // body.appendChild(loginModal);
            document.querySelector('#nodebb').appendChild(loginModal);
            var registerModal = prepareModal(data.registerModalTemplate, data.token, onSubmitSignUp);
        ...
        // SORTING COMPONENT
        nodebbDiv.querySelector("a[data-component='sort/best']").addEventListener('click', () => setSorting('best'));
        nodebbDiv.querySelector("a[data-component='sort/newest']").addEventListener('click', () => setSorting('newest'));
        nodebbDiv.querySelector("a[data-component='sort/oldest']").addEventListener('click', () => setSorting('oldest'));
...
function checkImgProfile() {
    if (document.querySelector('.first-image img')) {
        if (document.querySelector('.first-image img').getAttribute('src') == '') $('.first-image img.profile-image').remove();
        else $('.first-image div.profile-image').remove();
    }
}
...
export function createNestedComments(comments, template, otherData) {
    ...
    function createNestedCommentsInternal(comment, level) {
        ...
        clone
            .querySelector('.topic-profile-pic')
            .querySelector('a')
            .setAttribute('href', relativePath + '/user/' + comment.user.userslug);
